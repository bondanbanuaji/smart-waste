    ...
    useSSE((update: SSEDataUpdate) => {
        ...
        setDevices(prev => {
            if (!isPing && !isCleanup && update.wasteType) {
                const alertKey = update.wasteType === "ORGANIC" ? "sampah_basah" : "sampah_kering";
                    ...
                    <DialogContent className="sm:max-w-[500px] bg-white dark:bg-slate-950 border-slate-100 dark:border-slate-800 shadow-2xl">
                        <DialogHeader>
                            ...
                            <DialogDescription className="text-slate-500 dark:text-slate-400">
                                Mencari alat di jaringan lokal secara otomatis.
                            </DialogDescription>
                        </DialogHeader>
                ...
                <Dialog open={isDialogOpen} onOpenChange={(open: boolean) => !open && closeDialog()}>
                    ...
                    <DialogContent className="sm:max-w-[425px]">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                {selectedDevice ? "Perbarui informasi unit yang sudah terdaftar." : "Masukkan detail baru untuk dihubungkan ke sistem."}
                            </DialogDescription>
                        </DialogHeader>
                        <form onSubmit={handleSave}>
                            ...
                            <DialogFooter>
                                <Button type="button" variant="outline" onClick={closeDialog} disabled={isSubmitting}>Batal</Button>
                    ...
                    <DialogContent className="sm:max-w-[400px]">
                        <DialogHeader>
                            ...
                            <DialogDescription className="py-2">
                                Tindakan ini akan menghapus permanen alat <strong>{devices.find(d => d.id === isDeleting)?.name}</strong>.
                            </DialogDescription>
                        </DialogHeader>
                        ...
                        return (
                            <div key={device.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden hover:shadow-md transition-shadow group relative">
                                <div className="p-5 border-b border-slate-50 dark:border-slate-800 relative z-10">
                                    <div className="flex justify-between items-start mb-2">
                                        ...
                                        <div className="flex items-center gap-1.5">
                                            <Link href={`/dashboard/devices/${device.id}`}>
                                                <button className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-950/30 rounded-lg transition-colors">
                                                    <Settings2 className="h-3.5 w-3.5" />
                                                </button>
                                            </Link>
                                            <button onClick={() => openEditDialog(device)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950/30 rounded-lg transition-colors">
                                                <Pencil className="h-3.5 w-3.5" />
                                            </button>
                                            <button onClick={() => setIsDeleting(device.id)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-lg transition-colors">
                                    ...
                                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 line-clamp-1">{device.location}</p>
                                </div>
